...
function App() {
  ...
  const [
    ropePositionOnChain,
    maxScoreDifferenceOnChain,
    team1ScoreOnChain,
    team2ScoreOnChain,
    winStatusOnChain,
  ] = (data as readonly { result: bigint }[]) || [];
  ...
  const flagOffset =
    ropePositionOnChain?.result && maxScoreDifferenceOnChain?.result
      ? (Number(ropePositionOnChain.result) *
          40) /
          Number(maxScoreDifferenceOnChain.result) *
        5
      : ropePosition * 5;

  console.log(
    "Rendering with rope position:",
    ropePosition,
    "Flag offset:",
    flagOffset
  );
    ...
    <div className="tug-of-war-container">
      ...
      <div className="tug-of-war-score-board">
        <div className="tug-of-war-team1-score">
          Team 1 scores: {team1ScoreOnChain?.result ?? team1Score}
        </div>
        <div className="tug-of-war-team2-score">
          Team 2 scores: {team2ScoreOnChain?.result ?? team2Score}
        </div>
      </div>

      {winStatusOnChain && Number(winStatusOnChain.result) === 1 && (
        <h1 className="tug-of-war-title">Team 1 Wins</h1>
      )}
      {winStatusOnChain && Number(winStatusOnChain.result) === 2 && (
        <h1 className="tug-of-war-title">Team 2 Wins</h1>
      )}
      ...
      <div className="m-4">
        <p>
          [OnChain] ropePosition: {ropePositionOnChain?.result ?? 0},
          team1Score: {team1ScoreOnChain?.result ?? 0},
          team2Score: {team2ScoreOnChain?.result ?? 0}
        </p>

        {isConfirming ? <p>Waiting for confirmation...</p> : null}
